...
cd $SUEZ_DIR
python3 ./suez --show-chan-ids --client=c-lightning | grep '|' | tr -s ' ' | awk 'BEGIN { FPAT = "([[:space:]]*[[:alnum:][:punct:][:digit:]]+)"; } { printf "%16s [%13s <=> %-13s] f: %3s g: %8s ",$NF,$1,$3,$5,$9; } { $1=$2=$3=$4=$5=$6=$7=$8=$9=$NF=""; print $0}' >temp.status
